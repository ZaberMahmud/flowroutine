...
function selectTask(task) {
    ...
    const isInRoutine = state.routineTasks.some(t => t.id === task.id);
    ...
    selectedTaskInfo.innerHTML = `
        ...
        ${isInRoutine ? `<p><strong>Status:</strong> <span style="color: #10b981;">In Routine</span></p>` : ''}
    `;
    ...
    if (isInRoutine) {
        ...
        taskHour.value = currentHour > 22 ? 6 : currentHour;
        taskMinute.value = 0;
    }
...
function updateSelectedTask() {
    ...
    if (taskIndex !== -1) {
        ...
        if (checkTimeConflict(taskIndex)) {
            ...
            // Revert to old time
            state.routineTasks[taskIndex].startTime = oldStartTime;
            return;
        }
...
function checkTimeConflict(taskIndex) {
    ...
    for (let i = 0; i < state.routineTasks.length; i++) {
        ...
        if ((taskStart >= otherStart - buffer && taskStart < otherEnd + buffer) ||
            (taskEnd > otherStart - buffer && taskEnd <= otherEnd + buffer) ||
            (taskStart <= otherStart - buffer && taskEnd >= otherEnd + buffer)) {
